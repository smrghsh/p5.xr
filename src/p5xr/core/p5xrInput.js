...
import p5 from 'p5';
import { getEuler } from '../utilities/utilities';
...
export default class p5xrInput {
  ...
  constructor(inputSource, frame, refSpace) {
    ...
    if (this._targetRayPose) {
      ...
      // this._rot = getEuler(this._rot, this._targetRayPose.transform.orientation)
    }
  ...
  get rotation(){
    this.updatePose();
    const r = getEuler(vec3.create(),quat.create(this._pose?.transform?.orientation))
    console.log(r);
    return new p5.Vector(r.x,r.y,r.z);
